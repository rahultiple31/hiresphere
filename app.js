...
let feed = [
  ...
  },
  {
    ...
    meta: "18 proposals · escrow ready"
  },
  {
    initials: "RM",
    name: "Rahul M.",
    ...
    meta: "STAR score insight · 6 comments"
  }
];
  ...
  $("#bestMatchList").innerHTML = jobs.slice(0, 3).map((job) => `
    <article class="compact-card">
      <div>
        ...
        <small>${job.company} · ${job.location} · ${job.salary} LPA</small>
      </div>
  ...
  $("#pipelineList").innerHTML = candidates.map((candidate) => `
    <article class="candidate-row">
      <div>
        ...
        <small>${candidate.role} · ${candidate.skills}</small>
...
function renderProjects() {
  ...
  $("#invoiceList").innerHTML = invoices.map((invoice) => `
    <article class="invoice-item">
      ...
      <small>${invoice.amount} · ${invoice.state}</small>
    </article>
  `).join("");
}
...
function renderFeed() {
  $("#feedList").innerHTML = feed.map(feedTemplate).join("");
  $("#feedPreview").innerHTML = feed.slice(0, 2).map(feedTemplate).join("");
}
...
function initEvents() {
  ...
  $("#notifyToggle").addEventListener("click", () => $("#notificationTray").classList.toggle("open"));
  ...
  document.body.addEventListener("click", (event) => {
    ...
    const bidId = event.target.dataset.bid;
    const shortlistName = event.target.dataset.shortlist;
  ...
  $("#publishPost").addEventListener("click", () => $("#postComposer").requestSubmit());
  $("#startInterview").addEventListener("click", startInterview);
  $("#endInterview").addEventListener("click", endInterview);
  ...
  $("#recordSelfIntro").addEventListener("click", () => {
    notify("Self interview recording", "2-5 minute self-introduction capture started.");
  });

  $("#saveProfile").addEventListener("click", () => notify("Profile saved", "Candidate profile, documents, and video metadata were saved."));
